...
const exportReportPdf = window.exportReportPdf || (async function(){ alert('PDF no disponible.'); });
...
window.addEventListener('error', (event) => {
  ...
  if (boot && !boot.classList.contains('hidden')) {
    if (bootText) bootText.textContent = 'La app abrió en modo local con un error de carga. Revisa el enlace o vuelve a publicar.';
  }
});
...
function start() {
  ...
  const bootText = document.getElementById('bootText');
  if (bootText) bootText.textContent = 'Interfaz lista. Conectando sincronización...';
  setTimeout(() => el.bootCard.classList.add('hidden'), 120);
  const deferred = () => initFirebaseDeferred();
  if ('requestIdleCallback' in window) requestIdleCallback(deferred, { timeout: 1600 }); else setTimeout(deferred, 250);
}
